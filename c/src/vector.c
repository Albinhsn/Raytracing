...
Vec3f32 randomVec3f32InUnitDisk() {
  while (true) {
    Vec3f32 p = (Vec3f32){RANDOM_DOUBLE_IN_RANGE(-1, 1),
                          RANDOM_DOUBLE_IN_RANGE(-1, 1), 0};
    if (lengthSquaredVec3f32(&p) < 1) {
      return p;
    }
  }
...
inline void scaleVec3f32(Vec3f32 *restrict res, Vec3f32 *v, f32 t) {
  res->x = v->x * t;
  res->y = v->y * t;
  res->z = v->z * t;
}
inline void addVec3f32(Vec3f32 *restrict res, Vec3f32 *v0, Vec3f32 *v1) {
  res->x = v0->x + v1->x;
  res->y = v0->y + v1->y;
  res->z = v0->z + v1->z;
}
inline void subVec3f32(Vec3f32 *restrict res, Vec3f32 *v0, Vec3f32 *v1) {
  res->x = v0->x - v1->x;
  res->y = v0->y - v1->y;
  res->z = v0->z - v1->z;
}
inline void mulVec3f32(Vec3f32 *restrict res, Vec3f32 *v0, Vec3f32 *v1) {
  res->x = v0->x * v1->x;
  res->y = v0->y * v1->y;
  res->z = v0->z * v1->z;
}
void divideVec3f32(Vec3f32 *restrict res, Vec3f32 *v0, Vec3f32 *v1) {
  res->x = v0->x / v1->x;
  res->y = v0->y / v1->y;
  res->z = v0->z / v1->z;
}
inline f32 lengthVec3f32(Vec3f32 *v) { return sqrtf(lengthSquaredVec3f32(v)); }
inline f32 lengthSquaredVec3f32(Vec3f32 *v) {
  return v->x * v->x + v->y * v->y + v->z * v->z;
}
inline f32 dotVec3f32(Vec3f32 *v0, Vec3f32 *v1) {
  return v0->x * v1->x + v0->y * v1->y + v0->z * v1->z;
}
void crossVec3f32(Vec3f32 *restrict res, Vec3f32 *v0, Vec3f32 *v1) {
  res->x = v0->y * v1->z - v0->z * v1->y;
  res->y = v0->z * v1->x - v0->x * v1->z;
  res->z = v0->x * v1->y - v0->y * v1->x;
}
inline void normalizeVec3f32(Vec3f32 *res, Vec3f32 *v) {
...
void refractVec3f32(Vec3f32 *res, Vec3f32 *uv, Vec3f32 *n, f32 etaiOverEtat) {
  ...
  scaleVec3f32(&rOutParallel, n,
               -sqrt(fabs(1.0 - lengthSquaredVec3f32(&rOutPerp))));

  addVec3f32(res, &rOutPerp, &rOutParallel);
}

inline void reflectVec3f32(Vec3f32 *res, Vec3f32 *v, Vec3f32 *n) {
...
inline bool nearZeroVec3f32(Vec3f32 v) {
  f32 s = 1e-8;
...
void writeColor(Color v, i32 samples) {
  ...
  printf("%d %d %d\n", (i32)(256 * clamp(i, r)), (i32)(256 * clamp(i, g)),
         (i32)(256 * clamp(i, b)));
}

void debugVec3f32(Vec3f32 v) { printf("%lf %lf %lf\n", v.x, v.y, v.z); }
